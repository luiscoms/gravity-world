...
game.HUD.Joystick = me.Container.extend({
    init : function() {
        this._super(me.Container, 'init');
    },
    update : function() {
...
game.HUD.ItemCount = me.Container.extend({
    init: function (settings) {
        // call the parent constructor
        this._super(me.Container, 'init');

        var x = settings.x || 0, y = settings.y || 0, z = 1;
        x = x < 0 ? me.video.renderer.getWidth() + x : x;
        y = y < 0 ? me.video.renderer.getHeight() + y : y;

        if (settings.centerX) {
            x = (me.video.renderer.getWidth() / 2) - (me.loader.getImage(settings.image).width / 2);
        }
        if (settings.centerY) {
            y = (me.video.renderer.getHeight() / 2) - (me.loader.getImage(settings.image).height / 2);
        }
...
game.HUD.GUI_Button = me.Container.extend({
    init: function (settings) {
        // call the parent constructor
        this._super(me.Container, 'init');
